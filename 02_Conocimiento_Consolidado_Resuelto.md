...
### 18.24 Alineación de Infrastructure de Mi Plan con la matriz de RLS ya aprobada (corrección arquitectónica derivada de la auditoría técnica del Sprint 3.3.4)

**Contexto:** la auditoría técnica del Sprint 3.3.4 (Infrastructure Layer de Mi Plan) detectó que el 100% de las operaciones usa `withServiceContext()` (rol `dashboard_service_role`, BYPASSRLS), mientras que la regla MUST 5.5 ("Arquitectura de Datos — nivel conceptual", PRD Cap. 4: "el estudiante solo podrá acceder a su propia información"), operacionalizada en `docs/modules/dashboard.md` ("el [...] Service debe ejecutar sus lecturas en el contexto de sesión del estudiante autenticado, nunca con una credencial de servicio de alcance global, salvo en procesos programados de refresco"), y la migración ya aplicada `202607171400_my_plan_rls_policies` (Sprint 3.3.1, verificada con PGlite) exigen lo contrario para las operaciones que el propio estudiante inicia directamente. Una segunda auditoría (previa a esta resolución) confirmó con evidencia de código — no solo de prosa — que: (a) el puerto `UnitOfWork` (`features/my-plan/application/ports/UnitOfWork.ts`) ya nombraba `withStudentContext` en su propio comentario de diseño desde el Sprint 3.3.3, sin que su firma transportara el dato que ese mecanismo exige; (b) `services/auth/index.ts` (`requireAuthenticatedStudentId`) ya resuelve la identidad del estudiante autenticado vía Clerk en cada punto de entrada del proyecto, siguiendo el único patrón de propagación de identidad ya aprobado (parámetro explícito, sin contexto implícito); y (c) los 14 Handlers de Mi Plan ya poseen el `StudentId` (Value Object validado) exactamente en el punto donde invocan `UnitOfWork.execute()` o llaman a un Repository directamente — el problema es exclusivamente de transporte entre dos capas ya aprobadas, no de descubrimiento de identidad.

**Decisión 1 — mecanismo de transporte:** el puerto `UnitOfWork.execute<T>(work: () => Promise<T>): Promise<T>` se amplía a `execute<T>(work: () => Promise<T>, studentId?: string): Promise<T>`. Cuando se provee `studentId`, la implementación de Infrastructure debe usar `withStudentContext(studentId, ...)`; cuando se omite, conserva `withServiceContext(...)` sin cambios. No se introduce ningún objeto de contexto nuevo (`ExecutionContext`/`SecurityContext`/`RequestContext`): el dato transportado es el mismo `StudentId` que Domain y Application ya validan, sin nueva abstracción.

**Decisión 2 — alcance exacto de aplicación, según la matriz de GRANT/políticas ya migrada (no una interpretación nueva):** se invoca con `studentId` (→ `withStudentContext`, rol `dashboard_app_role`) exclusivamente en las operaciones cuyas escrituras Prisma están *íntegramente* contenidas en tablas donde `dashboard_app_role` ya tiene el `GRANT` y la política `*_self_*` correspondientes:
   - Las 5 Queries de lectura (`GetActiveLearningPlan`, `GetDailyPlan`, `GetWeeklyPlan`, `GetLearningProgress`, `GetStudySchedule`) — `dashboard_app_role` tiene `GRANT SELECT` ya vigente sobre las 7 tablas de escritura de Mi Plan y sobre `daily_plan`/`weekly_plan`/`learning_progress` (migraciones `202607170900` y `202607171400`), sin ningún caso de tabla mixta en una lectura.
   - `CreateStudySession` (escribe únicamente `study_session`, con `GRANT INSERT, UPDATE` explícito para `dashboard_app_role`).
   - `UpdateStudySchedule` (escribe únicamente `study_schedule`, con `GRANT UPDATE` explícito para `dashboard_app_role`).

   Permanecen en `withServiceContext` (studentId omitido), documentado explícitamente como alcance no cerrado por esta resolución:
   - `CreateLearningPlan`, `PauseLearningPlan`, `ResumeLearningPlan`, `CancelLearningPlan` — escriben `learning_plan`, tabla sobre la que `dashboard_app_role` **no tiene ningún `GRANT` de escritura** (la migración `202607171400` solo se lo otorgó a `dashboard_service_role`, pese a que `docs/modules/mi-plan.md`, punto 11, y 18.20 ya prometían "escritura real del propio estudiante sobre su fila" para esta tabla — promesa nunca ejecutada en migración; deuda de Sprint 3.3.1, no de este Sprint).
   - `CompleteLearningTask` (escribe `learning_task`, con `GRANT` para `dashboard_app_role`, **y** `learning_phase` dentro de la misma transacción, tabla de escritura exclusiva de servicio) y `UpdateLearningObjective` (escribe `learning_objective`, con `GRANT`, **y** `learning_goal` dentro de la misma transacción, también exclusiva de servicio) — un único rol Postgres rige toda la transacción; dividir estas dos operaciones en dos transacciones separadas para poder aplicar `withStudentContext` solo a la parte permitida excede la corrección mínima de esta resolución y queda fuera de alcance.
   - `RequestPlanReorganization` — no usa `UnitOfWork` (no escribe nada; ver 18.23), fuera de alcance de esta resolución por no compartir el mecanismo que aquí se corrige.

**Justificación DDD:** `StudentId` no es un dato nuevo introducido en el dominio ni en la aplicación — es el mismo Value Object que `StudentId.create(request.studentId)` ya produce en cada Handler (Sprint 3.3.3). Esta resolución no le añade responsabilidad ni lo transporta a una capa que no debería conocerlo: Application ya lo posee; solo se le permite pasarlo al puerto que ya lo necesitaba.

**Justificación Clean Architecture / Ports & Adapters:** ampliar la firma de `UnitOfWork` con un parámetro opcional no invierte la dirección de dependencia (Infrastructure sigue implementando un contrato que Application define) ni exige que Infrastructure "descubra" identidad por su cuenta — sigue siendo un adaptador puro que recibe lo que necesita a través de su puerto. Es la corrección mínima: un parámetro más en un contrato ya existente, no un mecanismo nuevo.

**Consistencia con CQRS:** la decisión trata lecturas y escrituras de forma independiente, tal como exige CQRS — las 5 Queries se resuelven con el mismo mecanismo (`UnitOfWork.execute` con `studentId`) sin necesidad de abrir una transacción de escritura real (Prisma permite transacciones de solo lectura); los 2 Command Handlers migrados no alteran su modelo de escritura, solo el rol de conexión bajo el que se ejecuta.

**Restricciones:** no se modifica ninguna política RLS ni ningún `GRANT` ya migrado; no se ejecuta ninguna migración nueva; no se modifica `schema.prisma`; no se modifica `Domain` (ninguna interfaz de Repository cambia); `OwnershipVerificationService` permanece sin cambios y se declara explícitamente aquí como complemento de RLS (verificación de autorización en Application Layer), no como sustituto — RLS y `OwnershipVerificationService` operan como capas independientes de defensa en profundidad, tal como ya caracteriza `docs/modules/dashboard.md` a RLS en su Actualización 4.

**Deuda reconocida, explícitamente fuera de alcance de esta resolución:** `learning_plan` (y por extensión `daily_plan`/`weekly_plan`/`learning_progress`, que dependen de su relación con `learning_plan`) carecen de política de escritura para `dashboard_app_role` — la migración de RLS que 18.20 prometió para estas 4 tablas nunca se ejecutó. Cerrar esa deuda requiere una migración nueva (fuera de alcance: "No modificar Prisma... salvo bug crítico") y, potencialmente, dividir `CompleteLearningTask`/`UpdateLearningObjective` en transacciones separadas por tabla — ambas decisiones quedan pendientes de una resolución futura si se decide cerrarlas.

**Módulos impactados:** `features/my-plan/application/ports/UnitOfWork.ts` (firma ampliada), 5 Handlers de consulta (inyección de `UnitOfWork`), `CreateStudySessionHandler`/`UpdateStudyScheduleHandler` (paso de `studentId` a una llamada ya existente), `features/my-plan/infrastructure/persistence/unit-of-work/PrismaUnitOfWork.ts` y `features/my-plan/infrastructure/composition/myPlanContainer.ts`. No afecta a Domain Layer (Sprint 3.3.2) ni a Data Model (Sprint 3.3.1, sin migraciones nuevas). Afecta artefactos ya tageados de Sprint 3.3.3 (`v1.4-application-layer`) y Sprint 3.3.4 en staging, ambos corregidos por esta misma resolución antes de su cierre formal.
