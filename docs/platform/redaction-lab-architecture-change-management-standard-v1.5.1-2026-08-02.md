...
| 1.1 | 2026-08-02 | ACP-011 (corrección retroactiva de cita — el "ACP-008" originalmente citado en esta fila nunca existió como archivo del repositorio; ver Investigación Forense v1.0 y Registro de Ejecución de ACP-011) | Incorporación del mecanismo de regularización retroactiva en las nueve secciones autorizadas por "ACP-008" (§3, §4, §6, §7, §9, §10, §11, §12, §13) — cita original, preservada aquí tal como fue escrita en su momento, por tratarse de un registro histórico. **La Auditoría Final de Congelación de v1.1 detectó que esta versión excedió el alcance autorizado**: modificó indebidamente VALIDACIÓN FINAL y RESULTADO, y añadió una sección nueva no autorizada ("Riesgos abiertos") que transcribía de forma incompleta los riesgos de ACP-008; además, omitió resolver la decisión sobre límite temporal exigida por el Plan de actualización de ACP-008, y dejó ambigua la aplicabilidad retroactiva de la calificación del §11 frente a ACP-006 v1.1. Dictamen de esa auditoría: B) Requiere correcciones. |
...
| 1.3 | 2026-08-02 | Revisión independiente de Standard v1.2 (hallazgos C-07, C-08) | Reescrita la calificación de irretroactividad del §11 (elegibilidad determinada por la fecha de presentación del ACP de Regularización, nunca por la fecha de la modificación original; ACP-006 v1.1 permanece rechazado, sin reapertura automática). Eliminada la justificación de "actos simultáneos" en §6.1, sustituida por ausencia de evidencia documental de un plazo. **La Auditoría de Cierre de v1.3 detectó una inconsistencia estructural**: los cinco desenlaces oficiales de §6.1 no correspondían íntegramente con los cuatro valores del campo "Estado del ACP" de §7, ni con la cadena de trazabilidad de §9, ni con el checklist de §13 — en particular, "regularización parcial" no tenía ningún valor representable. Dictamen de esa auditoría: B) Requiere correcciones. |
| 1.4 | 2026-08-02 | Auditoría de Cierre de v1.3 (hallazgos C-09, C-10, C-11) | Ampliado el campo "Estado del ACP" (§7) con el valor `APROBADO PARCIALMENTE`, con nota de correspondencia exacta frente a los cinco desenlaces de §6.1 (los cuales fueron anotados individualmente con su valor de Estado correspondiente); actualizada la cadena de trazabilidad de §9 para reflejar los cinco desenlaces; ampliado el checklist de §13 exigiendo coherencia entre Decisión (§6.1) y Estado del ACP (§7). **La Auditoría de Cierre de v1.4 detectó que el paso 5 de §6 (flujo normal, no modificado en ese ciclo) seguía enumerando únicamente tres veredictos (`APROBADO`, `RECHAZADO`, `APROBADO CON CONDICIONES`), sin reflejar el nuevo valor `APROBADO PARCIALMENTE` ya vigente en §7** — contradicción textual entre ambas secciones. Dictamen de esa auditoría: B) Requiere correcciones. |
| 1.5 | 2026-08-02 | Auditoría de Cierre de v1.4 (hallazgos C-12, C-13) | El paso 5 de §6 fue corregido para incluir `APROBADO PARCIALMENTE` en la enumeración de veredictos posibles, con una nota aclaratoria de que ese valor es exclusivo de ACP de Tipo Regularización (§6.1) — corrección mínima, sin alterar el funcionamiento del flujo normal ni introducir reglas nuevas. Este mismo Historial de cambios fue actualizado para reflejar con precisión dicha corrección. **La Auditoría de Cierre de v1.5 detectó, además, un hallazgo heredado de la propia v1.0**: VALIDACIÓN FINAL afirmaba "los tres criterios de rechazo automático (Sección 11)" cuando el §11 vigente —desde v1.0— siempre contuvo siete. Dictamen de esa auditoría: B) Requiere correcciones. |
| 1.5.1 | 2026-08-02 | ACP-011 (corrección retroactiva de cita — el "ACP-009 v1.1 (APROBADO)" originalmente citado en esta fila nunca existió como archivo del repositorio; ver Investigación Forense v1.0 y Registro de Ejecución de ACP-011) | Corregida, en VALIDACIÓN FINAL, la frase "los tres criterios de rechazo automático" por "los siete criterios de rechazo automático" — cambio descrito en su momento como autorizado por "ACP-009 v1.1", cita original preservada aquí tal como fue escrita, por tratarse de un registro histórico. Ninguna otra sección, palabra o formato fue modificado respecto de v1.5. |
| 1.5.1 (Corrección de citas — ACP-011) | 2026-08-02 | ACP-011 (APROBADO) | Ejecutado el Paso 7 del flujo oficial (§6) sobre ACP-011: corregidas las citas de atribución de la línea 6 ("Alcance de este documento") y de las filas "1.1" y "1.5.1" del Historial de cambios, que citaban a "ACP-008" y "ACP-009 v1.1 (APROBADO)" — ninguno de los cuales existe como archivo del repositorio (Investigación Forense v1.0) — para que citen correctamente a ACP-011 (`docs/platform/acp-011-regularizacion-retroactiva-autorizacion-real-v1.1-2026-08-02.md`, APROBADO), fundamento documental real y verificable: Architecture Review de ACP-011 (`docs/audits/architecture-review-acp-011-2026-08-02.md`, EMITIDA) y Registro de Ejecución de ACP-011 (`docs/platform/acp-011-registro-de-ejecucion-2026-08-02.md`). El texto narrativo original de las filas "1.1" y "1.5.1" (que describe qué se hizo en cada una de esas versiones) se preserva sin alterar, por constituir registro histórico de lo que efectivamente se escribió en su momento — únicamente se corrigió la columna "ACP relacionado", que identifica la autorización real. Sin ninguna otra modificación de contenido normativo — ninguna regla, definición, flujo, sección, resolución, ejemplo, checklist, tabla, numeración o mecanismo del Standard fue alterado. |
| 1.5.1 (Congelación) | 2026-08-02 | Auditoría de Cierre de Standard v1.5.1 (dictamen A) APROBADO) | Congelación documental: el documento transiciona de `MINOR REVISION` a `FROZEN`. Sin ninguna modificación de contenido normativo — únicamente se actualiza el campo "Estado" del encabezado y se añade esta fila, cerrando formalmente el flujo del §6 (paso 9, Congelación) tras la Auditoría de Cierre (paso 7) que confirmó el correcto cierre de ACP-009 v1.1. |
| 1.5.1 (Apertura de MINOR REVISION) | 2026-08-02 | ACP-010 v1.4 (APROBADO) | Ejecutado el Paso 1 del Plan de actualización de ACP-010: el campo "Estado" del encabezado cambia de `FROZEN` a `MINOR REVISION`, reconociendo formalmente que este documento tiene una decisión ya aprobada (ACP-010) pendiente de integrarse mecánicamente en su texto (Standard §3). Fundamento documental: ACP-010 v1.4 (`docs/platform/acp-010-regularizacion-ruptura-documental-cadena-gobernanza-v1.4-2026-08-02.md`, campo "Estado del ACP" = `APROBADO`); Architecture Review Board (ACCB) — Revisión final de ACP-010 v1.3 (`docs/audits/architecture-review-acp-010-v1.3-2026-08-02.md`, Estado documental `EMITIDA`, Confirmación expresa: A) APROBADO); Registro de Ejecución de ACP-010 (`docs/platform/acp-010-registro-de-ejecucion-2026-08-02.md`). Sin ninguna otra modificación de contenido normativo — ninguna regla, definición, flujo, sección, resolución, ejemplo, checklist, tabla, numeración o mecanismo del Standard fue alterado. El Paso 2 del Plan de actualización de ACP-010 (presentación del ACP nuevo y real que autorice de forma verificable el mecanismo de Regularización Retroactiva y la corrección de VALIDACIÓN FINAL) queda pendiente, no ejecutado en este acto. |
| 1.5.1 (Congelación tras ACP-011) | 2026-08-02 | Auditoría de Cierre de ACP-011 (dictamen A) satisfactoria) | Congelación documental: el documento transiciona de `MINOR REVISION` a `FROZEN`. Verificado, antes de este cambio: trazabilidad (línea 6 y filas "1.1"/"1.5.1" citan correctamente a ACP-011, con evidencia real y persistida); consistencia (ninguna otra fila ni sección alterada); referencias (Architecture Review de ACP-011 y Registro de Ejecución de ACP-011, ambos verificados como archivos reales); ausencia de modificaciones fuera del alcance del Plan de actualización de ACP-011 (§3–§14, VALIDACIÓN FINAL y RESULTADO permanecen idénticos); integridad general del documento. Sin ninguna otra modificación de contenido normativo — únicamente se actualiza el campo "Estado" del encabezado y se añade esta fila, cerrando formalmente el flujo del §6 (paso 9, Congelación) tras la Auditoría de Cierre (paso 8) que confirmó el correcto cierre de ACP-011. Con esta Congelación, la dependencia documental de "ACP-008" y "ACP-009" —confirmada inexistente por la Investigación Forense v1.0— queda definitivamente cerrada mediante autorización real y verificable (ACP-010, ACP-011), sin reconstrucción ni fabricación de registro histórico. |
...
5. **Aprobación.** El board emite un veredicto formal — `APROBADO`, `RECHAZADO`, `APROBADO CON CONDICIONES`, o `APROBADO PARCIALMENTE` (este último, exclusivo de ACP de Tipo Regularización, §6.1) — documentado con el mismo formato ya usado en este proyecto para las resoluciones A-01–A-10 ("Estado: APROBADA").
...
6. **Decisión.** Uno de los desenlaces oficiales siguientes, cada uno con su valor correspondiente de "Estado del ACP" (§7):
   - **Regularización aprobada, sin condiciones** (`Estado del ACP: APROBADO`).
   - **Regularización aprobada con condiciones** — exige ajuste editorial antes de la Congelación (`Estado del ACP: APROBADO CON CONDICIONES`).
   - **Regularización parcial** — solo la porción del cambio itemizada y verificada en el anexo de Comparación Línea por Línea se ratifica; cualquier porción no itemizada exige, según corresponda, corrección (paso 7) o reversión de esa porción específica (`Estado del ACP: APROBADO PARCIALMENTE`).
   - **Regularización rechazada, con reversión obligatoria** a la última versión `FROZEN` legítima anterior a la modificación no autorizada (`Estado del ACP: RECHAZADO`).
   - **Regularización rechazada, con apertura de un nuevo ACP prospectivo** — el documento se revierte primero; el mismo cambio puede volver a proponerse siguiendo el flujo oficial (Sección 6) en el orden correcto (`Estado del ACP: RECHAZADO`, con la distinción respecto al desenlace anterior documentada en los campos "Resultado esperado" y "Plan de actualización" del propio ACP, sin un valor de Estado adicional).
...
9. **Congelación o Reversión.** Si se aprueba (íntegra o parcialmente), el documento retorna a `FROZEN`, con el ACP de Regularización citado en su Historial de cambios (§12), señalando expresamente su naturaleza retroactiva. Si se rechaza, el documento se revierte obligatoriamente a su última versión `FROZEN` legítima.
...
| **Estado del ACP** | `PENDIENTE DE APROBACIÓN` / `APROBADO` / `APROBADO CON CONDICIONES` / `APROBADO PARCIALMENTE` / `RECHAZADO`. |

**Nota de correspondencia para ACP de Tipo Regularización**: para un ACP de Tipo Regularización (§4), los cinco valores del campo "Estado del ACP" corresponden exactamente a los cinco desenlaces oficiales del Flujo de Regularización Retroactiva (§6.1): `APROBADO` ↔ regularización aprobada sin condiciones; `APROBADO CON CONDICIONES` ↔ regularización aprobada con condiciones; `APROBADO PARCIALMENTE` ↔ regularización parcial; `RECHAZADO` ↔ regularización rechazada, ya sea con reversión obligatoria o con apertura de un nuevo ACP prospectivo — la distinción entre estos dos últimos sub-casos se documenta mediante los campos "Resultado esperado" y "Plan de actualización" del propio ACP, sin requerir un valor de Estado adicional. Ningún desenlace de §6.1 queda sin un valor correspondiente en este campo.
...
ACP de Regularización (ID único, con anexo de Comparación Línea por Línea)
        ↓
Decisión (§6.1) — uno de los cinco desenlaces oficiales
        ↓
Nueva versión (vX.(Y+1), FROZEN, Historial de cambios citando el ACP de Regularización)
   — íntegra, si Estado del ACP = APROBADO o APROBADO CON CONDICIONES
   — parcial, si Estado del ACP = APROBADO PARCIALMENTE (incorporando únicamente la porción ratificada por el anexo de Comparación Línea por Línea; la porción no ratificada se corrige o revierte antes de la Congelación, conforme al paso 7 de §6.1)
        — o —
Reversión a vX.Y (FROZEN, sin cambios respecto a la última versión legítima), si Estado del ACP = RECHAZADO — ya sea de forma definitiva o con apertura de un nuevo ACP prospectivo (Sección 6), según lo documentado en el propio ACP
```
...
- [ ] **Si el ACP es de Tipo Regularización (§4):** el anexo de Comparación Línea por Línea está presente y completo; la divulgación fue voluntaria y anterior a cualquier auditoría externa (§6.1, §11); la evaluación dual de boards (§10.1) fue ejecutada por ambos boards, no solo por uno; y el valor asignado al campo "Estado del ACP" (§7) corresponde exactamente al desenlace oficial de la Decisión (§6.1) alcanzado — incluyendo, cuando corresponda, el valor `APROBADO PARCIALMENTE`.
...
| ✓ Evita modificaciones directas a documentos Frozen | **Cumple.** El estado FROZEN (Sección 3) es explícitamente inmutable salvo vía ACP aprobado; los siete criterios de rechazo automático (Sección 11) cierran cualquier intento de omitir el proceso. |
