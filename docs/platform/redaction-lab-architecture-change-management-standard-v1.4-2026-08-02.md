# RÉDACTION LAB — ARCHITECTURE CHANGE MANAGEMENT STANDARD v1.4
...
| 1.3 | 2026-08-02 | Revisión independiente de Standard v1.2 (hallazgos C-07, C-08) | Reescrita la calificación de irretroactividad del §11 (elegibilidad determinada por la fecha de presentación del ACP de Regularización, nunca por la fecha de la modificación original; ACP-006 v1.1 permanece rechazado, sin reapertura automática). Eliminada la justificación de "actos simultáneos" en §6.1, sustituida por ausencia de evidencia documental de un plazo. **La Auditoría de Cierre de v1.3 detectó una inconsistencia estructural**: los cinco desenlaces oficiales de §6.1 (aprobada sin condiciones, aprobada con condiciones, rechazada con reversión, rechazada con apertura de nuevo ACP, regularización parcial) no correspondían íntegramente con los cuatro valores del campo "Estado del ACP" de §7, ni con la cadena de trazabilidad de §9, ni con el checklist de §13 — en particular, "regularización parcial" no tenía ningún valor representable. Dictamen de esa auditoría: B) Requiere correcciones. |
| 1.4 | 2026-08-02 | Auditoría de Cierre de v1.3 (hallazgos C-09, C-10, C-11) | **C-09:** ampliado el campo "Estado del ACP" (§7) con el valor `APROBADO PARCIALMENTE`, y añadida una nota de correspondencia exacta entre los cinco valores del campo y los cinco desenlaces de §6.1 (los dos desenlaces de rechazo comparten el valor `RECHAZADO`, distinguidos mediante los campos "Resultado esperado"/"Plan de actualización" del propio ACP, sin introducir un valor adicional para ese matiz). **C-10:** actualizada la cadena de trazabilidad de §9 para reflejar explícitamente los cinco desenlaces dentro de sus dos ramas ya existentes (Nueva versión / Reversión), sin introducir una tercera rama. **C-11:** ampliado el ítem de checklist de §13 específico para Tipo Regularización, exigiendo expresamente la coherencia entre el desenlace de la Decisión (§6.1) y el valor asignado a "Estado del ACP" (§7). Ningún estado documental, ningún board, ningún tipo de ACP ni el mecanismo de regularización en sí fueron modificados; ninguna decisión de ACP-008 fue alterada; ACP-006 v1.1 no fue reinterpretado. |
...
**Nota de correspondencia para ACP de Tipo Regularización** *(añadida, v1.4, C-09)*: para un ACP de Tipo Regularización (§4), los cinco valores del campo "Estado del ACP" corresponden exactamente a los cinco desenlaces oficiales del Flujo de Regularización Retroactiva (§6.1): `APROBADO` ↔ regularización aprobada sin condiciones; `APROBADO CON CONDICIONES` ↔ regularización aprobada con condiciones; `APROBADO PARCIALMENTE` ↔ regularización parcial; `RECHAZADO` ↔ regularización rechazada, ya sea con reversión obligatoria o con apertura de un nuevo ACP prospectivo — la distinción entre estos dos últimos sub-casos se documenta mediante los campos "Resultado esperado" y "Plan de actualización" del propio ACP, sin requerir un valor de Estado adicional. Ningún desenlace de §6.1 queda sin un valor correspondiente en este campo.
...
**Cadena alternativa — Regularización Retroactiva** *(actualizada, v1.4, C-10)*:
