# ACADEMIA — INFRASTRUCTURE MODEL v1.2
...
| 1.2 | 2026-07-29 | ACP-004 | Confirmado que `AttemptRepository` (implementado como `PrismaAcademyReadModelPort`) produce `versionCount` de `AttemptSummaryDTO` (documentado desde Application Model v1.5/API Contract v1.4 sin productor de Infraestructura confirmado hasta esta revisión) — se deriva del conteo de la relación `Attempt.versions` ya existente en el Domain Model v1.1 (Sección 3), sin extensión de firma de `save`/lecturas y sin ningún Repository ni método nuevo (Sección 5). Ejecución formalizada en `acp-004-registro-de-ejecucion-2026-07-29.md`. Ningún adaptador, Repository, patrón de eventos ni decisión ya resuelta por el IRB fue modificado más allá de lo aquí registrado. |
...
- `AttemptRepository`: `findById`, `findActiveByUnit`, `save` (persiste el Aggregate `Attempt`, incluyendo sus `Draft`/`Version`/`Feedback` internos). Debe soportar carga parcial para evitar el riesgo H-06 ya cerrado en el Domain Model v1.1 (no cargar indefinidamente todas las `Version`/`Feedback` de un `Attempt` — carga bajo demanda/paginada de historial). *(v1.1, ACP-001-A)*: `AttemptRepository.save` ya cubre, sin extensión, la persistencia de `currentStep` requerida por `CMD-16 AdvanceStep`/`CMD-17 VerifyComprehension` (Application Model v1.1) — no se agrega ningún método ni repositorio nuevo. *(v1.2, ACP-004)*: `AttemptRepository` (implementado como `PrismaAcademyReadModelPort`) produce `versionCount` de `AttemptSummaryDTO` (Application Model v1.5, Sección 6) derivándolo del conteo de la relación `Attempt.versions` ya existente en el Domain Model v1.1 — mediante `_count: { select: { versions: true } }` sobre la misma consulta Prisma ya usada por sus tres rutas de lectura de intento (continuación, historial por unidad, detalle de continuación). Ningún método nuevo, ninguna firma modificada, ningún Repository nuevo.
