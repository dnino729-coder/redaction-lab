# Academia Application Model v1.1

**Contrato de dominio congelado (no modificado):** `docs/audits/academia-domain-model-v1.1-2026-07-19.md`. **Fecha original:** 2026-07-19. **Fecha de esta revisión:** 2026-07-19.
...
**CMD-11 — AssignUnitToStudent** — **PENDIENTE DE DECISIÓN DE ARQUITECTURA**
- Propósito declarado por A-10: el Profesor "puede asignar unidades específicas de Academia a un estudiante o a un grupo completo".
- El Domain Model v1.1 **no modela** ninguna entidad, evento, Policy ni Specification para "asignación" — a diferencia de `FORCE_LOCK`/`FORCE_RESTART` (sí modelados en `OverrideAction`), no existe un concepto de dominio equivalente para "asignar". Este caso de uso **no puede orquestarse** sobre el Domain Model congelado sin inventar comportamiento de dominio no aprobado.
- Acción requerida antes de implementar: solicitar una extensión del Domain Model (fuera del alcance de este documento, que no puede modificarlo) que defina el concepto de "asignación" como dato/evento de dominio, o aclarar si "asignar" se resuelve enteramente como metadato de priorización sin afectar `UnitState`.
...
- Propósito: editar el contenido o comentario de IA de un `ModelExample` existente.
- Actor: Administrador.
...
**QRY-08 — GetGroupProgressSummary**
- Objetivo: exponer al Profesor el progreso agregado de Academia de un grupo completo (A-10).
- Filtros: `groupId` (identidad externa a Academia — contrato con el Bounded Context de Organización Académica, no detallado en el Domain Model — **PENDIENTE DE DECISIÓN DE ARQUITECTURA** el mecanismo exacto de resolución de membresía de grupo).
- Ordenamiento: por Estudiante.
- Paginación: **PENDIENTE DE DECISIÓN DE ARQUITECTURA** (tamaño de grupo no acotado en este documento).
- DTO de respuesta: `StudentProgressSummaryDTO[]`.
...
| AssignUnitToStudent | Asignar unidad | — | **PENDIENTE DE DECISIÓN DE ARQUITECTURA** | No modelado | No orquestable sobre v1.1 |
| CreateModelExample | Crear ejemplo modelo | `textType`, `content`, `rating`, `aiCommentary` | actor Administrador | `ModelExample` | Nuevo `ModelExample` |
...
| GetStudentProgressSummary | Progreso agregado (Profesor) | `studentId` | Por `TextType` | — | `StudentProgressSummaryDTO` |
| GetGroupProgressSummary | Progreso agregado de grupo | `groupId` | Por Estudiante | PENDIENTE | `StudentProgressSummaryDTO[]` |
...
**ModelExampleDTO** — `modelExampleId`, `textType`, `content`, `rating` (excelente/con errores), `aiCommentary`.
...
- Profesor: solo puede invocar `ApplyTeacherOverride`/`AssignUnitToStudent`/consultar `GetStudentProgressSummary`/`GetGroupProgressSummary` sobre estudiantes con relación docente ya establecida — mecanismo exacto de verificación de esa relación: **PENDIENTE DE DECISIÓN DE ARQUITECTURA** (pertenece al dominio de Organización Académica, §13.3, no a Academia).
...
**Paginación:** ver columna "Paginación" de la Sección 5 — la mayoría de las Queries de Academia operan sobre conjuntos acotados por estudiante/unidad, salvo `GetAttemptHistory` (potencialmente creciente sin límite, H-06) y `GetGroupProgressSummary` (tamaño de grupo no acotado en este documento) — ambas quedan marcadas **PENDIENTE DE DECISIÓN DE ARQUITECTURA** en cuanto a estrategia concreta de paginación.
...
**FUERA DEL ALCANCE DE ACP-001 (registrado, no resuelto en esta revisión):** `CMD-11 AssignUnitToStudent` conserva su marcador `PENDIENTE DE DECISIÓN DE ARQUITECTURA` de v1.0 en este documento, pese a que la Functional Specification v1.1 y el API Contract v1.0 ya reflejan la resolución ARB de "recomendar" (metadato, sin efecto de estado). `QRY-08 GetGroupProgressSummary` conserva su filtro `groupId`, pese a la decisión oficial de ACP-001-B de que no existe `GroupId` en el proyecto. Ninguno de los dos documentos fue listado como afectado por ACP-001-B en su alcance autorizado — su actualización queda pendiente de un ACP posterior.
