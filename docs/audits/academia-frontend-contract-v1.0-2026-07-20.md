# ACADEMIA — FRONTEND CONTRACT v1.0
...
**Fecha:** 2026-07-20.
**Estado:** DRAFT (pendiente del dictamen de esta misma auditoría, ver cierre del documento).

**Documentos Frozen consumidos como contrato obligatorio (ninguno modificado):** Product Blueprint, Arquitectura General, Domain Model v1.1, Application Model v1.2, Academia Functional Specification v1.2, Academia Infrastructure Model v1.1, Academia API Contract v1.1, Platform Core Foundation v1.0 (Notification Catalog), Academia Architecture Certified (`academia-architecture-certification-2026-07-19.md`).
...
| `/academy/teacher/students/{studentId}` | Progreso de un estudiante + acciones docentes: bloqueo/reinicio (CU-10), recomendar unidad (CU-11) | Profesor | `TEACHER` + verificación de relación docente-estudiante | `/academy/teacher` (selección de un estudiante) | Regreso a `/academy/teacher` | — |
...
- **Acciones disponibles:** forzar bloqueo (unidad en estado activo) o reinicio (unidad `COMPLETED`/`MASTERED`), con motivo obligatorio; recomendar una unidad (cualquier estado, incluido `LOCKED`).
...
**Nota de alcance — capacidad NO cubierta por este contrato:** las Secciones 2 y 6 de la Functional Specification describen, en su texto narrativo, que el Profesor puede "revisar todas las producciones, versiones e historial de retroalimentación de sus estudiantes". Esta capacidad **no forma parte de la definición formal de CU-09** (Sección 7 de la Functional Specification, que limita CU-09 al resumen agregado por estado) y, en consecuencia, **no tiene ningún endpoint correspondiente en el API Contract v1.1** (`EP-16`, `EP-17`, `EP-18` — historial de intentos, borrador y retroalimentación — están documentados exclusivamente con `Autorización: JWT + rol STUDENT + RLS`, sin rol `TEACHER`). Este Frontend Contract, por regla absoluta, no inventa un endpoint ni una pantalla para esta capacidad. Se registra como una inconsistencia interna heredada entre el texto narrativo y la definición formal de la Functional Specification v1.2, ya frozen — no corregible por este documento. Ver Validación Final y Dictamen.
...
- `AttemptHistoryRow` — fila de un intento previo en el historial.
...
flowchart TD
    ...
    D -- Revisar producciones detalladas --> D1["No soportado por el API Contract vigente\n(ver Sección 4, Nota de alcance)"]
...
- [ ] La capacidad "revisar producciones/versiones/retroalimentación detallada de un estudiante" (narrativa de las Secciones 2/6 de la Functional Specification) permanece explícitamente fuera de esta implementación, sin una pantalla sustituta improvisada — pendiente de un futuro ACP que la resuelva formalmente.
...
|---|---|
| ✓ Todas las pantallas derivan de la Functional Specification | **Cumple.** Las 14 pantallas (P-01 a P-14) trazan cada una a un CU explícito (CU-01 a CU-11) o a una función de soporte ya descrita (historial, continuidad, gestión editorial ya prevista para el Administrador en la Sección 2 FS). |
| ✓ Toda acción invoca un endpoint existente | **Cumple, con una excepción explícitamente registrada y no resuelta.** Las 21 pantallas/interacciones restantes invocan exclusivamente `EP-01` a `EP-22` del API Contract v1.1. La única excepción es la capacidad narrativa "revisar producciones/versiones/retroalimentación de un estudiante" (Secciones 2/6 FS), que **no tiene endpoint correspondiente** — este contrato no la implementa ni inventa uno; queda formalmente excluida (ver P-13, Nota de alcance). |
| ✓ No existen pantallas sin propósito | **Cumple.** Cada una de las 14 pantallas tiene un propósito trazado a un documento Frozen; ninguna fue añadida por conveniencia de diseño. |
| ✓ No existen componentes huérfanos | **Cumple.** Todo componente de la Sección 5 aparece referenciado desde al menos una pantalla de la Sección 4. |
| ✓ Toda navegación tiene respaldo funcional | **Cumple.** Cada ruta de la Sección 3 tiene un punto de entrada y una navegación posible derivados de la Sección 5/6 de la Functional Specification (flujo del estudiante/flujo del profesor). |
| ✓ No se inventaron funcionalidades | **Cumple.** Ninguna pantalla, componente o estado introduce una capacidad no descrita en algún documento Frozen; donde la Functional Specification narrativa excede lo que el API Contract permite (la excepción ya registrada), este documento se abstiene de inventar la solución, en vez de resolverla unilateralmente. |
| ✓ No se modificó ningún documento Frozen | **Cumple.** Ningún archivo de Product Blueprint, Arquitectura General, Platform Core Foundation, Domain Model, Application Model, Functional Specification, Infrastructure Model o API Contract fue editado durante la redacción de este documento. |

---

## RESULTADO

**B) REQUIRES FRONTEND REVIEW**

**Justificación con evidencia:** trece de las catorce pantallas de este contrato (P-01 a P-12, P-14) y diez de los once casos de uso de la Functional Specification (CU-01 a CU-08, CU-10, CU-11) quedan completamente cubiertos, con trazabilidad 1:1 hacia el API Contract v1.1 y sin ninguna funcionalidad inventada — la mayor parte del módulo está lista para implementación directa sin más revisión. Sin embargo, la auditoría automática de este mismo documento (Validación Final, segunda fila) identificó que la Functional Specification v1.2, en su texto narrativo (Secciones 2 y 6, no en la definición formal de CU-09, Sección 7), describe una capacidad del Profesor — "revisar todas las producciones, versiones e historial de retroalimentación de sus estudiantes" — para la cual el API Contract v1.1 no define ningún endpoint accesible al rol `TEACHER` (`EP-16`/`EP-17`/`EP-18` están explícitamente acotados a `STUDENT` + RLS). Por regla absoluta de este ejercicio ("no inventar funcionalidades", "toda acción invoca un endpoint existente"), este Frontend Contract no puede diseñar una pantalla o componente para esa capacidad sin exceder su alcance. En consecuencia, no puede emitirse el dictamen A) para el 100% del módulo: se requiere una revisión — no de este documento, sino de la cadena Functional Specification/API Contract que lo precede, vía un futuro Architecture Change Proposal que decida explícitamente si esa capacidad narrativa se formaliza (y se le añade el endpoint correspondiente) o se retira del texto de la Functional Specification por no ser parte del alcance real de CU-09. Hasta esa resolución, la implementación puede proceder con las 13 pantallas restantes sin bloqueo, dejando P-13 sin la funcionalidad de revisión detallada de producciones, tal como aquí se documenta.
