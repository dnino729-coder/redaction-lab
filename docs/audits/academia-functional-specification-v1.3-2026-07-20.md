...
**Estado:** FROZEN
**Documentos dependientes:** `academia-architectural-resolutions-v1.0-2026-07-19.md` (A-01 a A-10), `academia-domain-model-v1.1-2026-07-19.md` (Frozen), `academia-application-model-v1.3-2026-07-20.md`, `academia-domain-vs-application-audit-2026-07-19.md` (Change Proposal CH-01), `academia-arb-resoluciones-pendientes-2026-07-19.md` (Resolución ARB de los cinco pendientes funcionales), `redaction-lab-architecture-change-management-standard-v1.0-2026-07-19.md` (ACP-001-B, ACP-001-C, ACP-003).
...
| 1.1 | 2026-07-19 | — (ARB) | Integradas las cinco decisiones aprobadas por el Architecture Review Board. Eliminadas todas las marcas PENDIENTE DE DECISIÓN FUNCIONAL. Actualizadas Secciones 2, 6, 7 (CU-11), 8, 9, 11, 14, 16 y 17. Documento congelado (FROZEN). |
| 1.2 | 2026-07-19 | ACP-001-B, ACP-001-C | **ACP-001-B:** eliminada toda ambigüedad del término "grupo" — decisión oficial: no existe `Group` como entidad de dominio ni `GroupId`; toda referencia a "grupo" significa selección múltiple de estudiantes desde el Panel del Profesor. Actualizadas Secciones 2, 6, 7 (CU-09, CU-11), 15, 17. **ACP-001-C:** aclarado que el comentario comparativo de la Biblioteca de Modelos es contenido curatorial estático de autoría del Administrador, no generación dinámica de IA — corregida la Sección 10 (retirada de la lista de decisiones de IA) y las Secciones 4, 7 (CU-08). Ninguna regla funcional previa fue alterada; ninguna funcionalidad nueva fue añadida; el Domain Model y el Application Model (salvo su propia revisión v1.1 vía ACP-001-A, documento independiente) no fueron tocados por esta revisión. |
| 1.3 | 2026-07-20 | ACP-003 | **Teacher Review Visibility.** Añadida **CU-12 — Revisar historial académico detallado de un estudiante (Profesor)** (Sección 7), formalizando la capacidad que las Secciones 2 y 6 ya describían en su texto narrativo ("revisar producciones, versiones e historial de retroalimentación") sin que existiera una definición formal de Caso de Uso — eliminada la contradicción entre el texto narrativo y la lista formal de CU detectada durante la elaboración del Frontend Contract v1.0. Actualizadas las Secciones 2 (fila Profesor) y 6 (punto 2 del flujo) para referenciar CU-12 explícitamente. Ninguna regla funcional nueva fue introducida — la capacidad ya era parte oficial del producto desde v1.0 (texto narrativo), solo faltaba su formalización; el Domain Model y el Application Model (salvo su propia extensión v1.3 vía ACP-003, documento independiente) no fueron tocados por esta revisión. |
...
| **Profesor** | Observar el progreso agregado de sus estudiantes en Academia (individualmente o mediante selección múltiple); revisar todas las producciones, versiones e historial de retroalimentación de sus estudiantes, para una unidad específica (ver CU-12, *formalizado en v1.3, ACP-003*); forzar el bloqueo de una unidad ya desbloqueada; autorizar/forzar el reinicio de una unidad completada; **recomendar** unidades específicas a uno o varios estudiantes seleccionados (acción exclusivamente informativa: no modifica `UnitState` ni desbloquea unidades — ver CU-11). No existe `Group` como entidad de dominio en Academia — toda acción "sobre un grupo" es una selección múltiple de estudiantes individuales realizada por el Profesor desde el Panel del Profesor *(decisión oficial ACP-001-B, v1.2)*. No puede editar el contenido editorial de las unidades ni de la Biblioteca de Modelos — eso es exclusivo del Administrador. |
...
**Biblioteca de Modelos.** Colección de producciones ejemplares, propiedad exclusiva de Academia (A-04), consumida durante los pasos Observar/Analizar. Cada ejemplo incluye un comentario comparativo (por qué unas producciones puntúan mejor que otras) — **contenido editorial estático, redactado y mantenido por el Administrador** (§6.15), no generado dinámicamente por IA en el alcance actual *(aclarado en v1.2, ACP-001-C; ver Sección 10)*. Distinta de la Biblioteca temática de Laboratorio, sin superposición.
...
1. Accede al Espacio del Profesor y consulta el progreso agregado de Academia de un estudiante, o de varios estudiantes mediante selección múltiple (unidades bloqueadas/desbloqueadas/en curso/completadas/dominadas) — no existe un progreso "de grupo" calculado de forma nativa; el Panel del Profesor consulta el progreso de cada estudiante seleccionado *(ACP-001-B, v1.2)*.
2. Revisa las producciones de un estudiante: todas las versiones enviadas y toda la retroalimentación recibida en cada una, para una unidad específica que el estudiante ya haya iniciado (CU-12, *formalizado en v1.3, ACP-003*).
...
4. Puede recomendar unidades específicas a un estudiante o a varios estudiantes seleccionados: esta acción registra una prioridad visible para cada estudiante, sin modificar el estado de ninguna unidad ni desbloquearla (ver CU-11).
...
- Flujo principal: el estudiante consulta ejemplos del tipo de texto correspondiente, con comentario comparativo curatorial (Administrador, contenido estático — v1.2, ACP-001-C).
...
- Objetivo: visibilidad del avance de uno o varios estudiantes.
- Actor: Profesor.
- Precondiciones: relación docente establecida con cada estudiante consultado.
- Flujo principal: el Profesor consulta el resumen de unidades por estado de un estudiante, o de varios estudiantes mediante selección múltiple desde el Panel del Profesor — cada consulta se resuelve de forma independiente por estudiante; no existe un cálculo agregado nativo de "grupo" *(ACP-001-B, v1.2)*.
- Excepciones: sin relación docente establecida con alguno de los estudiantes seleccionados — acceso denegado únicamente para ese estudiante, sin bloquear la consulta del resto.
- Resultado: vista de progreso, por estudiante.
...
**CU-11 — Recomendar unidad a uno o varios estudiantes (Profesor)**
- Objetivo: destacar/priorizar una unidad específica para uno o varios estudiantes, como orientación pedagógica.
- Actor: Profesor.
- Precondiciones: relación docente establecida con cada estudiante destinatario.
- Flujo principal: el Profesor selecciona la unidad y uno o varios estudiantes destinatarios (selección múltiple desde el Panel del Profesor — no existe `GroupId` ni entidad `Group`, *decisión oficial ACP-001-B, v1.2*); el sistema registra una recomendación independiente por cada estudiante seleccionado, sin invocar ninguna transición de `UnitState`.
- Excepciones: sin relación docente establecida con alguno de los estudiantes seleccionados — acceso denegado únicamente para ese estudiante. La unidad recomendada puede encontrarse en cualquier estado (incluido `LOCKED`); la recomendación no altera ese estado.
- Resultado: la unidad aparece destacada/priorizada en el mapa de unidades de cada estudiante recomendado (p. ej., "Recomendada por tu profesor"), distinguible visualmente de las unidades efectivamente desbloqueadas; el estado real de la unidad permanece sin cambios.

**CU-12 — Revisar historial académico detallado de un estudiante (Profesor)** *(formalizado en v1.3, ACP-003 — Teacher Review Visibility)*
- Objetivo: consultar las producciones, versiones y retroalimentación recibidas por un estudiante en una unidad específica, así como el estado y el progreso de esa unidad (Sección 6, punto 2; capacidad ya descrita en el texto narrativo desde v1.0, sin definición formal hasta esta revisión).
- Actor: Profesor.
- Precondiciones: relación docente establecida con el estudiante consultado.
- Flujo principal: el Profesor selecciona un estudiante y una de sus unidades; el sistema presenta el estado actual de la unidad y su progreso, junto con el historial completo de intentos de esa unidad — cada intento con sus versiones enviadas y la retroalimentación recibida en cada una.
- Excepciones: sin relación docente establecida con el estudiante — acceso denegado; unidad seleccionada sin ningún intento iniciado por el estudiante — se presenta el estado de la unidad sin historial de intentos (lista vacía, no un error).
- Resultado: vista de solo lectura del historial académico del estudiante para la unidad seleccionada; ningún dato del estudiante es modificado por esta consulta.
...
- Nunca genera el comentario comparativo de la Biblioteca de Modelos — ese contenido es editorial estático, redactado y mantenido por el Administrador (Sección 4, CU-08) *(aclarado en v1.2, ACP-001-C — corrige una imprecisión de versiones anteriores; una futura generación dinámica por IA, si se aprueba, requeriría su propio ACP)*.
...
**Qué necesita Academia de otros módulos:** identidad y perfil del estudiante (Perfil); confirmación de si una unidad corresponde a una tarea planificada (Mi Plan, solo para decidir si notifica finalización); capacidad de generación de retroalimentación (Coach IA); señal de priorización de contenido (Motor Pedagógico); resolución de la relación docente-estudiante (Organización Académica) — necesaria para autorizar las acciones del Profesor descritas en la Sección 6, incluida cada selección múltiple de estudiantes (verificada individualmente por estudiante, no existe verificación de "grupo" — ACP-001-B, v1.2); sistema de notificaciones (para el aviso automático al finalizar retroalimentación diferida, Sección 11).
...
- [ ] Resolver el mecanismo de verificación de la relación docente-estudiante (contrato con Organización Académica) — sin membresía de grupo, dado que no existe `Group` como entidad (ACP-001-B, v1.2).
...
Los cinco puntos que impedían la congelación funcional del módulo en v1.0 (CU-11/recomendación, integración de "Editor", tiempos de espera de retroalimentación IA, comportamiento ante bloqueo docente en sesión activa, y nivel de accesibilidad) fueron resueltos de forma definitiva por el Architecture Review Board en v1.1. En v1.2, la Coverage Audit del módulo (`academia-architecture-coverage-audit-2026-07-19.md`) detectó dos imprecisiones adicionales — ambigüedad del término "grupo" (F-02) y confusión entre contenido estático e IA en la Biblioteca de Modelos (F-09) — resueltas mediante ACP-001-B y ACP-001-C, sin modificar el Domain Model, el Application Model (más allá de su propia revisión v1.1 vía ACP-001-A) ni ninguna resolución A-01–A-10.

En consecuencia, la **Especificación Funcional del módulo Academia queda oficialmente congelada en su versión 1.2** (posteriormente revisada a 1.3, ver más abajo). Los elementos que aún restan (cierre de CH-01, contrato de evidencia para `MASTERED`, verificación de relación docente-estudiante, contenido editorial inicial, reutilización de patrones de Infrastructure ya resueltos en Mi Plan, y verificación de consistencia del nivel WCAG con el Design System general) son tareas de cierre técnico u operativo — ninguna requiere una nueva decisión funcional — y se gestionan a través del checklist de la Sección 17, sin bloquear el Infrastructure Model ni el API Contract, ambos ya en curso.

En v1.3, la elaboración del Frontend Contract v1.0 detectó una contradicción interna del propio documento: el texto narrativo de las Secciones 2 y 6 describía una capacidad del Profesor ("revisar producciones, versiones e historial de retroalimentación de sus estudiantes") que nunca fue formalizada como Caso de Uso, dejándola sin endpoint ni pantalla posible. ACP-003 — Teacher Review Visibility resolvió la contradicción formalizando **CU-12** (Sección 7), sin introducir ninguna funcionalidad nueva — la capacidad ya era parte narrativa del producto desde v1.0.

---

## CERTIFICADO DE CONGELACIÓN

Se certifica que el presente documento, **Academia — Especificación Funcional v1.3**, constituye el **contrato funcional oficial** del módulo Academia para todas las fases posteriores del proyecto Rédaction Lab, incluyendo:
...
Ninguna modificación posterior a este documento podrá realizarse sin la emisión previa de un **Architecture Change Proposal (ACP)** formalmente aprobado, conforme al `Architecture Change Management Standard v1.0`.

**Estado:** FROZEN
**Versión congelada:** 1.3
**Fecha de congelación:** 2026-07-20
**Versión anterior (1.2):** pasa a estado `SUPERSEDED`.
