# Academia Application Model v1.5

**Contrato de dominio congelado (no modificado):** `docs/audits/academia-domain-model-v1.1-2026-07-19.md`. **Fecha original:** 2026-07-19. **Fecha de esta revisión:** 2026-07-29.
...
| 1.4 | 2026-07-20 | Reconciliación Documental (Sprint 4.2.2) | **R-02 (unificación de DTOs con el API Contract).** Sección 6 reescrita para que cada DTO compartido con el API Contract v1.3 tenga una única forma de campos, consolidada por unión (ningún campo previamente documentado en ninguno de los dos documentos fue eliminado; los conflictos de nombre se resolvieron adoptando un único nombre por campo). Afecta `AcademyUnitSummaryDTO`, `AcademyUnitDetailDTO`, `AttemptSummaryDTO`, `ContinuationStateDTO`, `DraftDTO`, `VersionDTO`, `FeedbackDTO`, `TeacherOverrideDTO`, `StudentProgressSummaryDTO`, `ModelExampleDTO`. Ningún Command, Query, Policy, Specification, Aggregate o regla de negocio fue modificado; el Domain Model no fue tocado. Corrección puramente documental, sin cambio de comportamiento ni de semántica de los datos. |
| 1.5 | 2026-07-29 | ACP-004 | **Confirmación de productor de `versionCount`.** El campo `versionCount` de `AttemptSummaryDTO` (Sección 6), documentado desde v1.4 sin productor confirmado en Infraestructura, queda formalizado como efectivamente entregado por el Read Model — se deriva del conteo de `Version` asociadas al `Attempt` vía la relación `Attempt.versions` ya existente en el Domain Model v1.1 (Sección 3, Entity `Version`). `QRY-05 GetVersionFeedback` permanece exactamente igual — ningún Aggregate, Value Object, Policy, Specification, Command o Query fue creado o modificado; el Domain Model no fue tocado. Corrección de contrato de datos, sin introducir ningún concepto de dominio nuevo. |
...
**Nota de reconciliación (v1.4, Sprint 4.2.2 — R-02/R-03):** los DTOs de esta sección son, a partir de esta revisión, **idénticos en forma** a su contraparte del mismo nombre en el API Contract v1.3, Sección 5 — unificados por unión de campos (ningún campo previamente documentado en cualquiera de los dos documentos fue eliminado) y con un único nombre por campo cuando existía un conflicto de nomenclatura. Esta sección es, desde v1.4, la **definición oficial** de cada DTO compartido; el API Contract v1.3 la referencia sin redefinirla.

**AcademyUnitSummaryDTO** — `unitId`, `studentId`, `textType`, `state` (`UnitState`), `position` (orden dentro de la progresión de su tipo de texto), `unlockedAt?`, `completedAt?`, `masteredAt?`, `attemptCount`, `eligibleForUnlock?` (booleano, cuando aplica a la Unidad siguiente), `repeatable?` (booleano), `isRecommended` (booleano, derivado de la existencia de una recomendación docente activa — no almacenado en el Aggregate).

**AcademyUnitDetailDTO** — extiende `AcademyUnitSummaryDTO` + `activeAttemptId?` (nulo si no hay intento en curso; antes `currentAttemptId` en el Application Model y `activeAttemptId` en el API Contract — unificado en `activeAttemptId`, v1.4), `attemptsCount` (total de intentos, ya presente en el API Contract), `teacherOverrideCount` (total de anulaciones docentes — concepto distinto de `attemptsCount`, ambos se conservan por representar datos diferentes).

**AttemptSummaryDTO** — `attemptId`, `unitId`, `currentStep` (`UnitStep`), `startedAt`, `isCurrent` (booleano), `versionCount`. **(v1.4, R-03: el campo `state` que aparecía únicamente en el API Contract v1.2 fue retirado — `Attempt` no posee, ni debe poseer, ningún estado propio distinto de `UnitState`; `currentStep` ya es, por diseño del Domain Model, la única información de posición del `Attempt` — Invariante 6.)** **(v1.5, ACP-004: `versionCount` queda confirmado con productor real en Infraestructura — se deriva del conteo de la relación `Attempt.versions` ya existente en el Domain Model, sin ninguna Query ni Command nuevo.)**

**ContinuationStateDTO** — `unitId?`, `attemptId?`, `currentStep?`, `draftContent?` (texto + conteo de palabras/caracteres), `lastSavedAt?` (antes `lastActivityAt` en el API Contract — unificado en `lastSavedAt`, v1.4, consistente con el mismo nombre ya usado en `DraftDTO`). Vacío/nulo si no existe continuidad activa. `draftContent` se preserva en la definición oficial porque es el único campo que permite a `EP-15` (API Contract) satisfacer por sí solo la restauración exacta de contenido exigida por A-06/Regla funcional 10 — su ausencia previa en el API Contract era una omisión documental, no una decisión de diseño (v1.4, R-02).

**DraftDTO** — `attemptId` (ya presente en el API Contract), `content`, `wordCount`, `characterCount` (v1.4: reincorporado — estaba presente en el Application Model v1.3 y ausente en el API Contract v1.2), `lastSavedAt` (antes `autosavedAt` en el Application Model v1.3 — unificado en `lastSavedAt`, v1.4).

**VersionDTO** — `versionId` (ya presente en el API Contract), `attemptId` (ya presente en el API Contract), `versionNumber`, `content`, `submittedAt` (antes `createdAt` en el Application Model v1.3 — unificado en `submittedAt`, v1.4, más preciso: coincide con el momento de envío, no de creación interna del Borrador), `feedbackStatus` (`"READY" | "PROCESSING"`, ya presente en el API Contract).

**FeedbackObservationDTO** — `category` (`FeedbackCategory`), `priority` (entero 1–10, H-07 del Domain Model), `strength` (`FeedbackStrength`), `explanation`, `suggestion`. *(Sin cambios en v1.4 — ya idéntico entre ambos documentos.)*

**FeedbackDTO** — `feedbackId` (ya presente en el API Contract), `versionId` (ya presente en el API Contract), `versionNumber` (se conserva del Application Model v1.3 — identifica la secuencia dentro del Intento sin requerir una segunda consulta a `VersionDTO`), `status` (`"READY" | "PROCESSING"`, ya presente en el API Contract), `observations: FeedbackObservationDTO[]`, `deliveredAt` (antes `generatedAt` en el API Contract v1.2 — unificado en `deliveredAt`, v1.4, alineado con el nombre del Domain Event `FeedbackDelivered` que lo origina).

**ModelExampleDTO** — `modelExampleId`, `textType`, `content`, `rating` (excelente/con errores; ya presente en el Application Model v1.3), `curatorialComment` *(v1.2, ACP-002-C — antes `aiCommentary`; contenido curatorial estático, autoría del Administrador vía `CMD-12`/`CMD-13`, no generado dinámicamente por IA en el alcance actual)*, `status` (`ACTIVE`/`RETIRED`; v1.4, R-02 — reincorporado desde el API Contract v1.2, donde ya existía sin contraparte en este documento; ambos campos se conservan por representar conceptos distintos: `rating` es una calificación editorial de calidad, `status` es el ciclo de vida de publicación).

**TeacherOverrideDTO** — `overrideId` (ya presente en el API Contract), `unitId` (ya presente en el API Contract), `action` (`OverrideAction`), `reason`, `appliedBy` (antes `authorId` en el Application Model v1.3 — unificado en `appliedBy`, v1.4), `appliedAt`.

**StudentProgressSummaryDTO** — `studentId`, `unitsByState` (conteo por cada uno de los 8 estados; antes descrito como "contadores por `UnitState`" en el Application Model v1.3 — unificado en el nombre ya usado por el API Contract, v1.4), `unitsByTextType` (conteo por cada uno de los 5 tipos de texto; antes "desglosado por `textType`" — mismo criterio de unificación de nombre).

**StudentUnitHistoryDTO** *(v1.3, ACP-003 — Teacher Review Visibility)* — `studentId`, `unitId`, `unitState` (uno de los 8 valores de `UnitState`), `attemptsCount`; `attempts`: lista de entradas compuestas exclusivamente por `AttemptSummaryDTO` (definición oficial de esta misma sección, v1.4) acompañado de sus `versions` — lista de `VersionDTO` (definición oficial de esta misma sección, v1.4) con `feedback` embebido cuando existe, mismo patrón ya usado en el Response contract de `EP-03`/`CMD-02`/`CMD-05`. Ningún campo de este DTO proviene de un concepto de dominio no expuesto previamente por otro DTO ya Frozen.
...
**FUERA DEL ALCANCE DE ACP-002 (registrado en su momento, resuelto en v1.4):** la discrepancia `rating`/`status` de `ModelExampleDTO` señalada aquí quedó **resuelta en v1.4** (Reconciliación Documental, Sprint 4.2.2, R-02) — ver Sección 6, ambos campos unificados y conservados. La referencia editorial a versiones desactualizadas del API Contract permanece fuera del alcance de este documento (corresponde corregirla en el propio API Contract).

**Resolución de residuos — Reconciliación Documental v1.4 (Sprint 4.2.2, R-02/R-03):** todos los DTOs compartidos con el API Contract quedan unificados en una única forma (Sección 6) — `AcademyUnitSummaryDTO`, `AcademyUnitDetailDTO`, `AttemptSummaryDTO` (con el campo `state` sin respaldo retirado, R-03), `ContinuationStateDTO` (con `draftContent` preservado, cerrando el riesgo sobre A-06/Regla funcional 10), `DraftDTO`, `VersionDTO`, `FeedbackDTO`, `TeacherOverrideDTO`, `StudentProgressSummaryDTO` y `ModelExampleDTO`. Ningún Aggregate, Policy, Specification, Domain Event, Command, Query o regla de negocio fue modificado — corrección exclusivamente de forma de DTO, sin cambio de comportamiento ni de semántica de los datos.
