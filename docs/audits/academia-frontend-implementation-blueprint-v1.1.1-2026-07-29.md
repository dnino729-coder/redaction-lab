# Academia — Frontend Implementation Blueprint v1.1.1

**Estado:** FROZEN
**Fecha:** 2026-07-29.
**Versión anterior:** v1.1 (FROZEN, 2026-07-28 — publicada bajo el nombre de archivo `academia-frontend-implementation-blueprint-v1.0-2026-07-28.md`, discrepancia de nomenclatura entre archivo y versión declarada corregida en esta revisión, ver AFR-038 hallazgo H-05) — auditada en AFR-001 (v1.0→v1.1) y en AFR-038 (esta revisión), hallazgos H-01 y H-05 de AFR-038 incorporados en esta versión.
**Autor:** Auditoría/planificación asistida (Claude Code), sin modificación de código.
**Basado en:** Frontend Contract v1.1 (FROZEN, 2026-07-20), API Contract v1.3 (2026-07-20), Application Model v1.4 (2026-07-20), Domain Model v1.1 (2026-07-19), Infrastructure Model v1.1 (2026-07-19), Functional Specification v1.3 (2026-07-20), Academia-Project-Freeze-Baseline.md, ACA-001/002/003-Report.md, Informe AFR-001, Informe AFR-038, y lectura directa del código en `features/academy/` (incluyendo `features/academy/actions/`, releído completo para esta versión), `app/api/v1/academy/`, `components/ui/`, `i18n/`, `middleware.ts`, `config/routes.ts`, `tailwind.config.ts`, `messages/{es,fr}.json`, `package.json`.
...
- **Navegación**: **precisión de esta versión (resolución AFR-F02)** — el botón "Reescribir" navega directo a `.../rewrite` (no requiere `EP-04`, el `currentStep` sigue en el ciclo de revisión). El botón **"Continuar a reflexión"** solo se habilita cuando el dominio lo permite (`currentStep === REWRITE` y al menos un ciclo de reescritura con feedback entregado — precondición real de `Attempt.advanceToReflection()`/`RevisionPolicy.assertMinimumCycleComplete`, verificada en código); al presionarlo, invoca `useAdvancePhase()` (EP-04) y, solo si resuelve con éxito, navega a `.../reflect` (P-10). Si `useAdvancePhase()` falla (ej. 409 porque la precondición no se cumple todavía), la navegación no ocurre y se muestra el error inline, permaneciendo en P-09.
  - **Aclaración (v1.1.1, AFR-038 — resuelve hallazgo H-01, contradicción interna de esta misma sección):** la frase anterior ("solo se habilita cuando el dominio lo permite") describe el efecto observable del flujo — el estudiante no logra avanzar de fase si la precondición de dominio no se cumple —, no una condición de deshabilitación client-side previa al clic. El criterio de aceptación 3 de esta misma pantalla (más abajo) exige literalmente que el botón sea presionable para que `useAdvancePhase()` pueda fallar con `409` y mostrar el error inline sin navegar; deshabilitarlo de antemano exigiría que el Frontend replicara `RevisionPolicy.assertMinimumCycleComplete()` — lógica de dominio que ningún DTO expone como booleano consultable —, violando el principio de esta misma especificación de que el Frontend no contiene lógica de negocio (Sección 2). El botón permanece habilitado salvo mientras la propia mutación `useAdvancePhase()` está en curso; la validación real de la precondición ocurre exclusivamente en el backend, exactamente como describe el criterio de aceptación 3.
...
| AFR-038, H-01 (Importante) | Cerrada la contradicción interna de P-09 entre la prosa de "Navegación" ("el botón solo se habilita cuando el dominio lo permite") y el criterio de aceptación 3 (que exige que el botón sea presionable para que `useAdvancePhase()` pueda fallar con 409 e informar inline) — se añadió una nota de aclaración que resuelve a favor del criterio de aceptación 3, ya implementado así en `VersionWithFeedbackPanel.tsx`, por ser la única lectura compatible con el principio de "Frontend sin lógica de negocio" (Sección 2). No se modifica ningún criterio de aceptación, componente ni comportamiento — solo se aclara la especificación. | 12 (P-09) |
| AFR-038, H-05 (Menor) | Corregida la discrepancia entre el nombre de archivo de la versión anterior (`...-v1.0-...md`) y su versión declarada en el encabezado (`v1.1`) — esta revisión adopta el nombre de archivo `academia-frontend-implementation-blueprint-v1.1.1-2026-07-29.md`, consistente con su propia versión. La versión anterior permanece sin modificar, conservada por trazabilidad histórica (`SUPERSEDED`). | Encabezado, nombre de archivo |
