# ACADEMIA — INFRASTRUCTURE MODEL v1.0
...
**Documentos Frozen consumidos como contrato obligatorio (no modificados, no reinterpretados, no extendidos):**
- Academia Functional Specification v1.1 (Frozen)
- Academia Domain Model — versión ratificada vigente (ver Nota de trazabilidad)
- Academia Application Model v1.0 (Frozen)
...
- `AttemptRepository`: `findById`, `findActiveByUnit`, `save` (persiste el Aggregate `Attempt`, incluyendo sus `Draft`/`Version`/`Feedback` internos). Debe soportar carga parcial para evitar el riesgo H-06 ya cerrado en el Domain Model v1.1 (no cargar indefinidamente todas las `Version`/`Feedback` de un `Attempt` — carga bajo demanda/paginada de historial).
- `ModelExampleRepository`: `findByTextType`, `findById`, `save`, `retire` (soft-delete, consistente con "ejemplo retirado" ya contemplado en CU-08 de la Functional Spec).
