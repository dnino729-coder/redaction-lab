...
**Fecha de esta revisión:** 2026-07-29
...
| 1.4 | 2026-07-29 | ACP-004 | **Confirmación de productor de `versionCount` y corrección de la URI de `EP-18`.** El campo `versionCount` de `AttemptSummaryDTO` (Sección 5), documentado desde v1.2 sin confirmación de productor en Infraestructura, queda formalizado como efectivamente entregado — se deriva del conteo de `Version` asociadas al `Attempt` vía la relación `Attempt.versions` ya existente en el Domain Model. Corregida la URI de `EP-18` (Sección 4) para reflejar el parámetro de query `versionNumber`, ya exigido por `QRY-05 GetVersionFeedback` (Application Model v1.5) y por la Infraestructura real (`getVersionFeedback`) desde antes de esta revisión — la v1.3 no lo documentaba en la URI, por omisión editorial. Ningún endpoint, DTO, Command, Query, Aggregate o regla de negocio fue modificado en su comportamiento; corrección puramente documental. |

**Documentos Frozen consumidos como contrato obligatorio (no modificados, salvo lo explícitamente registrado en el Historial de cambios de arriba):** Product Blueprint, Arquitectura General, Domain Model, Application Model v1.5, Academia Functional Specification v1.3, Academia Infrastructure Model v1.1, Platform Core Foundation v1.0 (en particular, el Notification Catalog y el tipo ya aprobado `ACADEMY_FEEDBACK_READY`). *(v1.3, R-09: referencias de versión actualizadas — antes citaba Application Model v1.1 y Functional Specification v1.1. v1.4, ACP-004: referencia de Application Model actualizada a v1.5.)*
...
- **Método / URI:** `GET /api/v1/academy/attempts/{attemptId}/feedback?versionNumber={number}` *(v1.4, ACP-004: parámetro de query `versionNumber` restituido en la URI documentada — `QRY-05 GetVersionFeedback` (Application Model) exige `versionNumber` como filtro obligatorio, y la Infraestructura real (`getVersionFeedback`) siempre lo requirió; la v1.3 omitía este parámetro en la URI por un error editorial, sin que el comportamiento real del sistema haya cambiado.)*
- **Autorización:** JWT + rol `STUDENT` + RLS.
...
**Nota de reconciliación (v1.3, Sprint 4.2.2 — R-02/R-03):** desde esta revisión, la definición oficial de cada DTO compartido con el Application Model es la de `academia-application-model-v1.5-2026-07-29.md` *(v1.4, ACP-004: referencia actualizada — antes citaba `academia-application-model-v1.4-2026-07-20.md`)*, Sección 6. Esta sección lista, por transparencia, la misma forma (sin redefinirla) e indica explícitamente cada corrección aplicada.
...
- **`AttemptSummaryDTO`**: `attemptId`, `unitId`, `currentStep` (uno de los 11 valores del `UnitStep`), `startedAt`, `isCurrent`, `versionCount`. *(v1.3, R-03: retirado el campo `state` — `Attempt` no posee ningún estado propio distinto de `UnitState` (Invariante 6 del Domain Model); `currentStep` ya es su única información de posición. Se incorporan `isCurrent`/`versionCount`, ya presentes en el Application Model v1.3.)* *(v1.4, ACP-004: `versionCount` queda confirmado con productor real en Infraestructura — se deriva del conteo de la relación `Attempt.versions` ya existente en el Domain Model, sin ninguna Query ni Command nuevo.)*
